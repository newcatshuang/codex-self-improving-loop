...
4. 在候选审阅页先用右侧审阅上下文确认来源、风险和下一步，再打开详情或审批抽屉查看证据、LLM 分析、建议文本和合并建议。
5. 先在审批抽屉预览晋升 diff，再在人工审批操作台确认是否写入。
...
- 白底指标卡展示候选、skill、运行记录和审阅队列的核心状态，避免把总览做成操作页。
- 运行状态条汇总常用动作的当前状态；具体写入、导出和重建动作仍在对应页面执行。
...
- 合并建议位于候选工具栏中，用于处理相似候选，避免重复晋升。
- 候选行展示类型、目的地、安全状态、审阅状态、优先级和更新时间。
- 点击候选行只刷新右侧审阅上下文，不会立即弹出抽屉。
- 审阅上下文显示当前候选的状态、来源、归属和建议下一步，并提供打开详情和打开审批入口。

使用方式：

1. 先选择一个候选，让右侧审阅上下文定位当前审阅对象。
2. 在上下文中确认来源、状态、安全标记、归属和建议下一步。
3. 点击打开详情，查看证据来源、候选文本和 LLM 分析是否足够稳定。
4. 对相似候选先处理合并建议，再决定是否晋升。
5. 需要写入时再打开审批，先预览 diff，再确认是否晋升。

### 3. 人工审批：先预览 diff，再确认写入

![WebUI 人工审批与 diff](docs/assets/webui-approval-diff.png)

人工审批操作台通常从右侧审阅上下文打开，也是唯一会触发晋升写入的位置：
...
数据中心使用左右工作台，把高频操作和恢复复盘分开：

- 左侧操作列按安全、导出、危险分组，避免把重建数据库放进日常按钮流里。
...
- 右侧洞察列集中展示恢复复盘队列和回滚预览，避免页面重复出现多个回滚入口。
...
![WebUI Skill 来源上下文](docs/assets/webui-skill-source-context.png)

Skill 管理页关注两件事：

- 安装或更新内置 `session-recall`、`memory-capture` skills。
- 查看 skill 相关候选、skill 使用次数、最近使用时间、补丁数和建议动作。
- 从 skill 候选点击查看或预览时，会进入候选审阅工作台，并在右侧上下文中标记来源和返回 skill 的入口。

使用方式：

1. 安装后或升级后点击 `安装 / 更新技能`，同步仓库内置 skills 到 `$HOME/.agents/skills`。
2. 如果 `USER.md` 不存在，可点击 `初始化 USER.md 模板`。
3. 对 skill 候选先点击查看或预览，确认触发条件、步骤、验证和安全边界。
4. 如果从 Skill 管理进入候选审阅，可使用右侧上下文里的返回入口回到 Skill 管理页。
5. 对 skill patch 候选先导出 patch，再人工检查目标 skill，避免自动改变代理行为。
